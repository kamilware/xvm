...
SYSTEMD_SERVICE_NAME = "xvm-go-autoupdate.service"
SYSTEMD_SERVICE_PATH = os.path.expanduser(f"~/.config/systemd/user/{SYSTEMD_SERVICE_NAME}")
SYSTEMD_SERVICE_CONTENT = f"""[Unit]
Description=XVM Go Auto Update

[Service]
ExecStart=%h/.local/bin/xvm go install latest
Restart=on-failure
...
LAUNCHD_LABEL = "xvm.go.autoupdate"
...
WINDOWS_TASK_NAME = "XVMGoAutoUpdate"
WINDOWS_COMMAND = f'"{XVM_EXEC_PATH}" go install latest'
...
def setup_autoupdate_service():
    ...
    if sys_info["os"] == "linux":
        ...
        plist = {
            "Label": LAUNCHD_LABEL,
            "ProgramArguments": [XVM_EXEC_PATH, "go", "install", "latest"],
            "RunAtLoad": True,
            "KeepAlive": False,
            "StandardOutPath": os.path.expanduser("~/Library/Logs/xvm-go-autoupdate.log"),
            "StandardErrorPath": os.path.expanduser("~/Library/Logs/xvm-go-autoupdate.err"),
        }
