...
def ensure_alias(version: str):
    ...
    path = os.path.join(XVM_NODE_DIR, version)
    s = f'alias node{version}="{path}/bin/node"\n'
...
def add_export(path: str):
    ...
    try:
        with open(rc_path, "a") as f:
            f.write(f'export NODE_HOME="{path}"\n')
            f.write(f'export PATH="$NODE_HOME/bin:$PATH"\n')
    except Exception as e:
        log_to_file('error', str(e))
...
def remove_export():
    ...
    try:
        ...
        with open(rc_path, "w") as f:
            for line in lines:
                if "NODE_HOME=" in line or "$NODE_HOME/bin" in line:
                    continue
                f.write(line)
    except Exception as e:
        error(f"Failed to clean old Node settings from {rc_path}")
        log_to_file('error', str(e))
...
def remove_alias(version: str):
    ...
    try:
        ...
        with open(rc_path, "w") as f:
            for line in lines:
                if "alias" in line and f"node{version}" in line:
                    continue
                f.write(line)
    except Exception as e:
        error(f"Failed to clean old Node settings from {rc_path}")
...
def get_path(version: str) -> Optional[str]:
    ...
    try:
        ...
        error(f"Failed to retrieve installation dir for {version}")
        log_to_file('error', str(e))
...
def activate(version: str):
    ...
    success(f'Node.js version {version} successfully activated, please reopen your terminal')
