...
from src.node.download import download
from src.node.list import list_available_versions
from src.node.rc import remove_export, add_alias, add_export
from src.node.utils import normalise_version
from src.shared.const import SUPPORTED_SHELLS, XVM_NODE_DIR
...
def install(version: str):
    ...
    install_path = os.path.join(XVM_NODE_DIR, version)
    ...
    try:
        with tarfile.open(archive_path, "r:xz") as tar:
            tar.extractall(path=XVM_NODE_DIR)

        extracted_dir = f"node-v{version}-{sys_info['os']}-{sys_info['arch']}"
        os.rename(os.path.join(XVM_NODE_DIR, extracted_dir), install_path)
        success(f"Node {version} installed to {install_path}")
    ...
    send_notification("XVM Node", f"node{version} installed successfully", sys_info)
